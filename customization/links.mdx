---
title: Profile Links
sidebarTitle: Profile Links
description: Learn how to add links to your profile on haunt.gg
icon: link
---

## How to add a link?

<Steps>
    <Step title="Access Links Page">
        Head over to [haunt.gg/dashboard/links](https://haunt.gg/dashboard/links) and click on ``Add New Link`.
    </Step>
    <Step title="Choose Your Platform">
        Once you choose which Platform to add, you can either select `Hyperlink` to add an URL or `Copyable Text` to add your username for people to copy.
    </Step>
    <Step title="Add Link">
        Once you pasted the link or username, click on `Add Link` and you're done!
    </Step>
</Steps>

<Frame>
    ![Links Add Interface](../images/links-add-interface.png)
</Frame>

## How to add a custom link?

<Steps>
    <Step title="Access Links Page">
        Head over to [haunt.gg/dashboard/links](https://haunt.gg/dashboard/links) and click on ``Add New Link`.
    </Step>
    <Step title="Choose Custom Platform">
        On the platform option, choose `Custom`.
    </Step>
    <Step title="Change Settings and Add Link">
        You can customize all settings, like title, icon an URL.
        It is also possible to use a copyable text, like with regular links.
        Once you pasted the link or text, click on `Add Link` and you're done!
    </Step>
</Steps>

<Frame>
    ![Links Custom Interface](../images/links-custom-interface.png)
</Frame>

## How to customize link icons?

### Edit settings for all links
    In the same location where you add your links, you can see the `Icon Settings`.
    Here you can change the icon color color, make the icons monochrome and add a glow effect.

<Frame>
    ![Links Icon Settings](../images/links-icon-settings.png)
</Frame>

### Edit color for each link individually
    If you want to have different colors for each link, you have to have the monochrome effect disabled.
    You can then click on the color palette icon to change the icon color.
    The color can be reset to the default one at all times.

<Frame>
    ![Links Icon Color](../images/links-icon-color.png)
</Frame>



## Supported Link Platforms

  <Expandable title="list of supported link platforms">
      - Snapchat
      - Youtube
      - Discord (Server)
      - Discord (User)
      - Spotify
      - Instagram
      - X (Twitter)
      - TikTok
      - Telegram
      - SoundCloud
      - PayPal
      - GitHub
      - GitLab
      - Twitch
      - Kick
      - Reddit
      - LinkedIn
      - Steam
      - Pinterest
      - Last.fm
      - Facebook
      - Patreon
      - Genius
      - OnlyFans
      - Roblox
      - NameMC
      - Stats.fm
      - Labynet
      - Bitcoin
      - Litecoin
      - Ethereum
      - Solana
      - Mail
      - Custom
    </Expandable>